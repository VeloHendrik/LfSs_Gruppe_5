...
class Node:
    ...
    def best_child(self, c_param=1.0, player=None):
        """
        Wählt das Kind mit dem höchsten UCB1-Wert plus einem progressiven Bias,
...
class MCTSAgent:
    ...
    def mcts_iteration(self, root, player):
        ...
        node = root
        while node.children and not node.state.is_terminal() and node.is_fully_expanded():
    ...
    def backpropagate(self, node, result):
        """
        Propagiert das Ergebnis entlang des Pfades zur Wurzel.
        Dabei wird abwechselnd der result-Wert und sein Komplement (1 - result) addiert,
        da sich die Perspektive zwischen den Spielern wechselt.
        """
        current_reward = result
        while node is not None:
            node.visits += 1
            node.wins += current_reward
            # Bei jedem Schritt wird die Perspektive gewechselt
            current_reward = 1 - current_reward
            node = node.parent
